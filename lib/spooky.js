...
Spooky.prototype._onCreate = function (callback, error, response) {
    ...
    this.thenGetData = function(casperFn, callbackFn) {
        if (typeof casperFn !== 'function' || typeof callbackFn !== 'function') {
            throw new Error('thenGetData arguments must be functions');
        }

        var eventName = 'getData' + this.getDataEventNo++;
        this.once(eventName, function (data) {
            callbackFn(new Buffer(data, 'base64').toString('utf8'));
        });
        this._call('getData', null, casperFn, eventName);
    };
